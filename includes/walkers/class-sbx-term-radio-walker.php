 ...
 */
class SBX_Term_Radio_Walker extends Walker {
	...
	public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $args['list_only'] ) ) {
			...
			$output .= "\n<li id='{$taxonomy}-{$category->term_id}'$class>" .
				'<label class="selectit"><input value="' . intval( $category->term_id ) . '" type="radio" name="'.esc_attr( $name ).'[]" id="in-'.esc_attr( $taxonomy ).'-' . intval( $category->term_id ) . '"' . '" data-value="'.esc_attr( $category->slug ) . '"' .
