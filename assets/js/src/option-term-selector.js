...
class OptionTermSelector extends Component {
	constructor() {
		super( ...arguments );
			...
			return (
				<div key={ term.id } className="editor-post-taxonomies__hierarchical-terms-choice">
					<option
						...
						selected={ terms.indexOf( term.id ) !== -1 }
                        value={ term.id }
                        data-value={ term.slug }
					>{ unescapeString( term.name ) }</option>
					{ !! term.children.length && (
						<optiongroup className="editor-post-taxonomies__hierarchical-terms-subchoices">
