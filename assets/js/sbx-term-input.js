(function($) {
	...
	$( document ).on( 'click', '#the-list .editinline', function(e){
		...
		// get the post ID
		var post_id = inlineEditPost.getId(e.target),
		...
		$('.post_category', rowData).each(function(){
			...
			if ( $('.'+taxonomy+'-checklist').prev( 'input' ).length ) {
				$('.'+taxonomy+'-checklist').prev( 'input' ).remove(); // remove the hidden tax_input input, prevents WP from running its default save routine
			}
